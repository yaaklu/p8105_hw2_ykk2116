...
```
_The number of distinct stations that serve the A train is `r nrow(reformatted_nyc_subway)` and `r sum(reformatted_nyc_subway$ada)` are ADA compliant.
...
_The total precipitation in 2017 was `r sum(precipitation_2017$total)` and the median number of sports balls was `r median(median_balls_2016$sports_balls)`
